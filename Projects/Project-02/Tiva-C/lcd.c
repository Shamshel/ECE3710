...
int getChar(char character, int height, int width)
{
	return (character - 0x20)*((height)/8)*width;
	
}

extern void lcd_drawChar(int _x, int _y, char character, unsigned short _color, const unsigned char* _font)
{
	int x = 0, y = 0, currentByte = 0;
	unsigned char currentChar = 0;
	int width = _font[0];
	int height = _font[1];
	unsigned char charStart = _font[2];
	unsigned char charEnd = _font[3];
	int offset = getChar(character, height, width) + 4;
	...
	if(charStart > character || charEnd < character)
	{
		return;
	...
	for(y = 0; y < height; y++)
	{
		...
		for(currentByte = 0; currentByte < width/8; currentByte++)
		{
			currentChar = _font[offset + currentByte + (width/8)*y];
			
			for(x = 0; x < 8 ; x ++)
			{				
				//check for pixel write
				if((currentChar>>(7-x))%2 == 1)
				{
					//lcd_drawBox(_x+x+currentByte*8, _y+y, 1, 1, _color);
					
					//Cursor at _x, _y
					writeReg(0x004f, _x + x + currentByte*8); // Set GDDRAM Y address counter
					writeReg(0x004e, 320-(_y + y)); // Set GDDRAM X address counter
					...
					gpio_digitalWrite(GPIO_E, GPIO_PIN_2,	0xFF);	//Low for latch lock
				...
				else
				{
					...
					gpio_digitalWrite(GPIO_B, 0xFF, 0x0000); //Lower 8 bits

					gpio_digitalWrite(GPIO_E, GPIO_PIN_2,	0xFF);	//Low for latch lock

					gpio_digitalWrite(GPIO_B, 0xFF, 0x00); //Upper 8 bits
...
void lcd_drawString(int _x, int _y, char* string, int len, unsigned short _color, const unsigned char* _font)
{
	int i = 0;
	int width = _font[0];
	for(i = 0; i < len; i++)
	{
		lcd_drawChar(_x+i*width, _y, string[i], _color, _font);
		
	}
	
}

char* lcd_makeString(unsigned int input, char* inArr)
{
	unsigned int tmp = input, i = 0;
	unsigned char curChar = 0;
	
	inArr[curChar] = tmp%10+0x30;
	curChar++;
